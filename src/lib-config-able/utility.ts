...
let animate = (speed: ANIMATE_SPEED_TYPE, animate: boolean = CORE_CONFIG['animate'], type: string= null): string => {
  if(type == 'file'){
    return  (' ') + (
      !getConfig('animate') || !animate ? 'file:tui-no-animate' :
      'file:tui-animate file:transition-all file:ease-in-out ' +
      (speed=='faster' ? 'file:duration-100' :
      speed=='fast' ? 'file:duration-150' : 
      speed=='slow' ? 'file:duration-500' :
      speed=='slower' ? 'file:duration-700' :
      'file:duration-300')
    )
  }
  return  (' ') + (
    !getConfig('animate') || !animate ? 'tui-no-animate' :
    'tui-animate transition-all ease-in-out ' +
    (speed=='faster' ? 'duration-100' :
    speed=='fast' ? 'duration-150' : 
    speed=='slow' ? 'duration-500' :
    speed=='slower' ? 'duration-700' :
    'duration-300')
  )
}

let rounded = (value: ROUNDED_TYPE|boolean, side:string = 'all'): string => {
  if(!getConfig('isRounded') || value===false || value=='none'){
